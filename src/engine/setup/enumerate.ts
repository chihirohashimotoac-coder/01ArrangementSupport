...
export function rankSetupRoutes(
   ...
   *
   * 主目標（既定 T20）そのものが安全なら、シングル落ち耐性を理由とした
   * 振り直しはしない。そのときの候補は安全な得点用トリプル全体で、
   * そのどれを狙うかは従来どおりビジット全体の評価
   * （残りの質 → 取得点 → 難易度）が決める。
   *
   * これは「主目標が安全なら必ず主目標から投げる」という意味ではない。
   * 残りの質は取得点より重い（docs/APPROVALS.md A-7）ので、T20 が安全でも
   * 別のトリプルの方が良い残しを作れる残り点では、そちらが第 1 候補になる。
   * 例: 279 / 3 本は T13 → S20 → T20（160 残し）で、T20 始動の最良
   * T20 → T20 → S20 は 139 残しにしかならない。この挙動は v1.3.4 以前からの
   * もので、ここで T20 を強制すると承認済みの重みが良いと評価する残しを
   * 捨てることになるため、そうしない（docs/APPROVALS.md A-18）。
   *
