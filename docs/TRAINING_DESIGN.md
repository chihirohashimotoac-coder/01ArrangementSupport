...
```
難易度 → trivial 上限 → 直近履歴の窓（5,3 → 5,1 → 5,0 → 3,0 → 1,0）→ カテゴリ
```

緩める順序は次のとおりです。

1. 難易度の希望 → **種別の難易度 quota が残っている難易度** → 制約なし（同じ bucket の中で）
2. trivial 上限（同じ bucket の中で）
3. 直近履歴の除外窓（5 問 / 3 問 → 5 / 1 → 5 / 0 → 3 / 0 → 1 / 0）
4. bucket を広げる（カテゴリを外す。最後の手段）

1 の中間段階（quota 残）が重要です。希望した難易度が出題順の制約で使えないとき、
