...
- ラスト 1 投の調整を**現在残りを主語に**固定する
  （182 → S18 → 164 / 183 → S19 → 164 / 185 → S18 → 167 /
  186 → S19 → 167 / 188 → S18 → 170 / 189 → S19 → 170、
  いずれも S20 では 162 / 163 / 165 / 166 / 168 / 169 のノーテンになること）
- 184 → S20 → 164、187 → S20 → 167 は無理にずらさせないこと
- 302〜309 と現在残り 182〜189 の対応が一致すること
- 同じ現在残りが複数の context から出題でき、学習タグが共通であること
...
3 本フルの組み立ては、これとは別の観点で見る（v1.3.7 / APPROVALS A-18）。
検証するのは **1 投目に何を狙うか**であって、完成ルートの固定ではない。

- 299 / 302 / 303 / 305 / 306 / 308 / 309 の第一ターゲットが
  `S18` / `S19` ではなく、18 / 19 系の**得点用トリプル**であること
- その第一ターゲットが同ナンバーのシングルへ落ちても、テンパイ経路が残ること
- 実着弾のあとは、その残り点・残り本数から計算し直すこと
  （299 → S19 なら 280 / 2 本 → `T20 → T20` → 160 残し）
- T20 が安全な 300 / 301 / 304 / 307 は、T18 / T19 へ振り替えないこと
- 通常 SETUP の第一ターゲットが、TRAINING / FIRST DART の推奨と一致すること
...
狭い出題範囲も専用スイープで確認する（独立監査 F-006 / F-008）。
171〜182 は 3 投フル候補 12 件がすべて同じカテゴリ・すべて HARD なので
「1 問目は HARD にしない」と 80/20 が衝突し、
正規化後のカテゴリ枠には候補が 1 件しかないものがあるので
カテゴリ quota と anti-repeat も衝突する。
どちらも候補不足ではない（1 投調整 120 件 / 3 投フル 12 件）ので、
**同時に満たすこと**を hard acceptance にする。

```
狭い出題範囲（171〜182 / 302〜309、10 問・30 問、復習 on / off）
  出題数（generated）           : 期待どおり
  SETUP の形式 quota 違反       : 0
  出題順の違反                  : 0
  直近 5 問の同じ problemKey    : 0（セッション単位・問題単位の両方）
  直近 3 問の同じ状況           : 0（同上）
  直前とまったく同じ問題        : 0
  trivial 上限超過              : 0
  同じ seed から別の並び        : 0
...
狭い範囲でカテゴリ枠に候補が足りないときは、**同じ問題を早く出し直すのではなく**
形式を保ったまま別カテゴリへ決定論的に広げる（`quotaNormalizedCount` に出る）。
default の出題範囲ではカテゴリ quota を崩さないことを、別途 hard acceptance にしている。

MIXED 30 問 + 復習も専用スイープで確認する（独立監査 F-010）。
200 seeds の復習監査では、発生率 0.4% の構成崩れを安定して検出できなかった。

```
MIXED 30 問 + 復習（reviewTargets = [122, 302]、1,000〜10,000 seeds）
  出題数（generated）           : 期待どおり
  SETUP カテゴリ quota 違反     : 0（セッション単位・件数単位の両方）
  SETUP 形式 quota 違反         : 0
  出題順の違反                  : 0
  種別 quota 違反 / 同一種別 3 連続 : 0
  直近 5 問 / 直近 3 問 / 直前と同じ : 0
  trivial 上限 / 1 投上がり上限  : 0
  復習枠が 0 のセッション        : 0
  苦手問題の露出                : 復習なしより増える
  同じ seed から別の並び        : 0
```
